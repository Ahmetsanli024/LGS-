...
const replacement = `const handleDownloadPdf = async () => {
    ...
    const isLandscape = sortedSchedule.length <= 7;
    ...
    clone.style.width = isLandscape ? Math.max(el.scrollWidth, 1123) + 'px' : Math.max(el.scrollWidth, 794) + 'px';
    ...
    try {
      ...
      noPrintElements.forEach(item => item.remove());
      ...
      pdf.save(\`Odev_Takip_\${studentName?.replace(/\\s+/g, '_') || 'Ogrenci'}.pdf\`);
    } catch (error) {
...
const filePath = './components/Module10.tsx';

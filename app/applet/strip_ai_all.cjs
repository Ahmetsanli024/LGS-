...
const modules = [
    ...
    './components/DistributionWizard.tsx' // Wait, removing from Distibution wizard might break the whole PDF parse function the user uses manually? Let's check distribution wizard later.
];
...
modules.forEach(file => {
    ...
    // If Distribution wizard, wait we should not destroy DistributionWizard UI except if we want to remove PDF parts completely.
    // The user said: "her modülde yapay zeka ile program oluştur kısımlarını kaldırabilir misin kod fazlalığı olmasın."

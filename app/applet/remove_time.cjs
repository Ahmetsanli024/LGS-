...
function removeTimeFromDistributionWizard() {
    ...
    if(fs.existsSync(f)) {
        let txt = fs.readFileSync(f, 'utf8');
        txt = txt.replace(/<div className="w-px h-8 bg-slate-200 hidden sm:block" \/>\s*<div className="flex flex-col">\s*<span className="text-\[10px\] font-bold text-slate-400 uppercase tracking-widest">Tahmini Günlük Süre<\/span>\s*<span className="text-xl font-black text-indigo-600 leading-tight">\s*\{hrs > 0 \? `\$\{hrs\}sa ` : ''\}\{mns\}dk\s*<\/span>\s*<\/div>/g, "");
        fs.writeFileSync(f, txt);
    }
}

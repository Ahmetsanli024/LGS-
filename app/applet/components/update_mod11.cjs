...
txt = txt.replace(/const lessonName = lessons\[offset\];/g, `const lessonMatch = lessons[offset].match(/^(.*?) (Matematik|Türkçe|Fen|İnk\\.Tar|İngilizce|Fen Bilg\\.|Din Kültürü|Sosyal Bilgiler|Fizik|Kimya|Biyoloji|Coğrafya|Tarih|Geometri|Edebiyat)$/i);
...
`{(item as any)?.teacher && (
                                                <div className="text-[10px] font-medium text-black uppercase tracking-wider text-center leading-tight mb-0.5 whitespace-nowrap overflow-hidden text-ellipsis w-full">
                                                    {(item as any).teacher}
                                                </div>
                                            )}
